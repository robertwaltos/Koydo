# Koydo — Content Rating Questionnaire Answers
**IARC (Google Play) + Apple App Store Age Rating**

---

## Google Play — IARC Content Rating

Complete the IARC questionnaire at:
`Google Play Console → App Content → Content Rating → Start Questionnaire`

### Category: Education

Select **Education** as the primary content category.

---

### Section 1: Violence
| Question | Answer |
|----------|--------|
| Does the app contain any violence? | **No** |
| Does the app contain references to violence in a humorous context? | **No** |
| Does the app contain cartoon violence? | **No** |
| Does the app contain realistic violence? | **No** |
| Does the app contain graphic/gory violence? | **No** |

---

### Section 2: Sexual Content
| Question | Answer |
|----------|--------|
| Does the app contain sexual content or nudity? | **No** |
| Does the app contain suggestive content? | **No** |

---

### Section 3: Language
| Question | Answer |
|----------|--------|
| Does the app contain profanity or crude humor? | **No** |
| Does the app allow users to post content visible to others? | **No** |

---

### Section 4: Controlled Substances
| Question | Answer |
|----------|--------|
| Does the app reference alcohol, tobacco, or drugs? | **No** |
| Does the app promote or facilitate use of controlled substances? | **No** |

---

### Section 5: User Interaction & User-Generated Content
| Question | Answer |
|----------|--------|
| Does the app allow users to interact with each other? | **Yes** (Discussion boards) |
| Can users interact with strangers? | **No** (moderated Q&A boards, no direct messaging) |
| Does the app contain user-generated content visible to others? | **Yes** (Q&A posts — moderated) |
| Does the app provide tools to report/block offensive content? | **Yes** |
| Does the app have privacy policy & COPPA compliance measures? | **Yes** |

---

### Section 6: Location Sharing
| Question | Answer |
|----------|--------|
| Does the app share user location with other users? | **No** |

---

### Expected IARC Rating Result
**Everyone (E) — No descriptors**

*Koydo will receive an E rating on Google Play based on these answers.*

---

## Apple App Store — Age Rating

Set in App Store Connect under `App Information → Age Rating`.

### Answer each question:

| Category | Setting |
|----------|---------|
| Cartoon or Fantasy Violence | **None** |
| Realistic Violence | **None** |
| Prolonged Graphic or Sadistic Realistic Violence | **None** |
| Profanity or Crude Humor | **None** |
| Mature/Suggestive Themes | **None** |
| Horror/Fear Themes | **None** |
| Medical/Treatment Information | **None** *(general educational content only)* |
| Alcohol, Tobacco, or Drug Use or References | **None** |
| Simulated Gambling | **None** |
| Sexual Content or Nudity | **None** |
| Graphic Sexual Content and Nudity | **None** |

### User-Generated Content
| Question | Answer |
|----------|--------|
| Unrestricted Web Access | **No** |
| Allows users to create accounts? | **Yes** |
| Allows users to post content visible to others? | **Yes** (moderated discussion boards) |
| Shares user-generated content with others? | **Yes** (Q&A posts) |

### Expected Apple Age Rating
**4+ (Ages 4 and up)**

*Selecting 4+ reflects that Koydo is appropriate for all ages and contains no restricted content.*

---

## COPPA Compliance Note

Koydo explicitly serves learners ages 3 and up, including children under 13. Per COPPA (Children's Online Privacy Protection Act):

- Users under 13 require verifiable parental consent before account creation.
- Koydo does not knowingly collect personal information from children under 13 without verifiable parental consent.
- **Parent consent flow is implemented** — age-gate middleware detects under-13 users and triggers the parental consent flow before any data collection.
- Parent consent can be revoked at any time via the parent dashboard or the `/api/compliance/parent-consent/revoke` endpoint, which triggers automatic data erasure.
- The app contains zero advertising, zero cross-app tracking, and never sells user data.
- Marketing targets all ages (Pre-K through adult learners, ages 3+).
- Privacy Policy URL: https://koydo.app/privacy

### Google Play Families Policy

Because Koydo explicitly targets children under 13:
- The app **must be enrolled in the Google Play Families Program** (Designed for Families).
- The app must comply with the Families Self-Certified Ads SDK Program requirements (Koydo has **zero ads**, so this is satisfied by default).
- Teacher Approved badge should be applied for (optional but recommended).
- Target age group in Play Console: **Ages 5 and under, Ages 6-8, Ages 9-12, Ages 13+** (all checked — the app is for everyone).

---

## GDPR / EU Compliance Note

- Legal basis for data processing: Consent + Contractual necessity
- Users in EU may request data deletion via Settings → Account → Delete Account
- Data processing disclosure in onboarding flow
- DPA (Data Processing Agreement) required for B2B / school customers
