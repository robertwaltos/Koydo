...
const RUN_REPORT_PATH = path.join(ROOT, "public", "GAMES-300-PIPELINE-RUN.json");
...
async function main() {
  ...
  const steps = [
    ...
    },
    {
      id: "factory-plan",
      command: "node",
      args: ["scripts/build-300-games-factory-plan.mjs", ...passthroughArgs],
    },
    {
  ...
  console.log(
    `[games-300-factory-pipeline] status=${report.status} steps=${stepResults.length} report=${relativeFromRoot(RUN_REPORT_PATH)}`,
  );
...
main().catch((error) => {
  console.error("[games-300-factory-pipeline] failed:", error);
  process.exitCode = 1;
});
