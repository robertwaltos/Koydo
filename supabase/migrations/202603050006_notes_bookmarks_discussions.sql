...
CREATE INDEX IF NOT EXISTS idx_user_notes_user ON user_notes(user_id, updated_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_discussion_module ON discussion_posts(module_id, created_at DESC);
ALTER TABLE discussion_posts ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Anyone can read discussions" ON discussion_posts;
CREATE POLICY "Anyone can read discussions" ON discussion_posts FOR SELECT USING (true);
DROP POLICY IF EXISTS "Auth users create discussions" ON discussion_posts;
CREATE POLICY "Auth users create discussions" ON discussion_posts FOR INSERT WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users delete own posts" ON discussion_posts;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_user_certificates_unique_combo
  ON user_certificates(user_id, certificate_type, COALESCE(module_id,''), COALESCE(exam_id,''));
CREATE INDEX IF NOT EXISTS idx_user_certificates_user ON user_certificates(user_id, issued_at DESC);
ALTER TABLE user_certificates ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Users read own certificates" ON user_certificates;
CREATE POLICY "Users read own certificates" ON user_certificates FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "System inserts certificates" ON user_certificates;
