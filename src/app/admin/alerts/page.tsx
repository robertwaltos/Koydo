...
export default async function AdminAlertsPage() {
      ...
      <main className="mx-auto flex w-full max-w-5xl flex-col items-center justify-center px-6 py-24">
        <SoftCard className="w-full max-w-2xl border-rose-200 bg-rose-50 p-8 text-center">
          ...
          <p className="mt-3 text-sm text-rose-700">
            You must be an administrator to access alerts.
          </p>
        </SoftCard>
      </main>
    );
  ...
  const openAlertCount = (alertsResult.data ?? []).filter((entry) => !entry.acknowledged).length;
    ...
    <main className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-6 py-12">
      <SoftCard as="header" className="border-accent/20 bg-[var(--gradient-hero)] p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <ProgressChip label="Open Alerts" value={openAlertCount} tone={openAlertCount > 0 ? "warning" : "success"} />
          <ProgressChip label="Notif Queued" value={initialNotificationSummary.queuedCount} tone="info" />
          <ProgressChip label="Notif Failed" value={initialNotificationSummary.failedCount} tone={initialNotificationSummary.failedCount > 0 ? "warning" : "neutral"} />
        </div>
      </SoftCard>
