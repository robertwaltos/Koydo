...
export async function GET() {
  ...
  if (error) {
    console.error("Failed to query admin audit logs report.", toSafeErrorRecord(error));
    return new Response(JSON.stringify({ error: "Failed to query admin audit logs report." }), {
      status: 500,
