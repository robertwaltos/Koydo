...
export const CpuGpuMemoryDesign101Module: LearningModule = {
  ...
  description:
    "Explore the internal architecture of modern processors and memory systems — from CPU instruction pipelines and cache hierarchies to GPU parallel execution models and performance optimization strategies.",
  subject: "Engineering",
  tags: ["core", "curriculum", "interactive", "engineering", "computer-architecture"],
  minAge: 16,
  maxAge: 99,
  version: "2.0.0",
  ...
  lessons: [
    ...
    /* ──────────────────────────────────────────────
       L02  Pipelining and Superscalar Design  (interactive)
    ────────────────────────────────────────────── */
    {
      ...
      interactiveActivities: [
        {
          ...
          buckets: ["Data Hazard", "Control Hazard", "Structural Hazard"],
          items: [
            { text: "SUB reads R1 before ADD writes to R1", bucket: "Data Hazard" },
            { text: "Pipeline fetches wrong path after a branch", bucket: "Control Hazard" },
            { text: "Two instructions both need the single ALU at the same time", bucket: "Structural Hazard" },
            { text: "MUL writes R3 before DIV finishes reading old R3 value", bucket: "Data Hazard" },
            { text: "Conditional jump outcome unknown until EX stage", bucket: "Control Hazard" },
            { text: "Instruction fetch and data load both access single-port memory", bucket: "Structural Hazard" },
            { text: "LOAD result not ready when next ADD needs it as input", bucket: "Data Hazard" },
          ],
        },
      ],
    },
    ...
    /* ──────────────────────────────────────────────
       L03  CPU Architecture Checkpoint  (quiz)
    ────────────────────────────────────────────── */
    {
      ...
      questions: [
        {
          ...
          options: [
            ...
            { id: "b", text: "Decode" },
            { id: "c", text: "Execute" },
            { id: "d", text: "Write-back" },
          ],
          correctOptionId: "c",
          explanation:
            "The Execute stage is where the ALU performs the actual computation. Fetch loads the instruction from memory, Decode interprets the opcode and reads registers, and Write-back stores the result.",
        },
        {
          ...
          options: [
            { id: "a", text: "2× speedup" },
            { id: "b", text: "5× speedup" },
            { id: "c", text: "10× speedup" },
            { id: "d", text: "25× speedup" },
          ],
          correctOptionId: "b",
          explanation:
            "A k-stage pipeline can ideally achieve k× throughput improvement. With five stages, the theoretical maximum is 5× because once the pipeline is full, one instruction completes every cycle instead of every five cycles.",
        },
        {
          ...
          skillId: "cpu-gpu-memory-design-101-skill-core",
          options: [
            { id: "a", text: "Branch prediction" },
            { id: "b", text: "Forwarding (bypassing)" },
            { id: "c", text: "Register renaming" },
            { id: "d", text: "Cache prefetching" },
          ],
          correctOptionId: "b",
          explanation:
            "Forwarding (also called bypassing) routes the ALU result directly to the next instruction's input before it is written back to the register file, avoiding the stall. Branch prediction addresses control hazards, and register renaming eliminates false (WAR/WAW) dependencies.",
        },
        {
          ...
          skillId: "cpu-gpu-memory-design-101-skill-core",
          options: [
            { id: "a", text: "It cannot support self-modifying code" },
            { id: "b", text: "It requires two separate memory systems" },
            { id: "c", text: "Instructions and data share a single bus, limiting bandwidth" },
            { id: "d", text: "It does not support virtual memory" },
          ],
          correctOptionId: "c",
          explanation:
            "The von Neumann bottleneck arises because a single bus is shared between instruction fetches and data accesses, preventing simultaneous access. Harvard architecture uses separate buses and memories for instructions and data, doubling effective bandwidth.",
        },
      ],
      quizBlueprint: {
        totalQuestions: 4,
      ...
      interactiveActivities: [
        {
          ...
          pairs: [
            { left: "Direct-mapped cache", right: "Each memory block maps to exactly one cache line" },
            { left: "Fully-associative cache", right: "A block can be placed in any cache line" },
            { left: "Set-associative cache", right: "A block maps to a set but can occupy any way within it" },
            { left: "Write-back policy", right: "Dirty data written to memory only on eviction" },
            { left: "Write-through policy", right: "Every write is immediately sent to the next memory level" },
            { left: "LRU replacement", right: "Evicts the cache line that was least recently accessed" },
          ],
        },
      ],
    },
    ...
    /* ──────────────────────────────────────────────
       L06  Memory Hierarchy Checkpoint  (quiz)
    ────────────────────────────────────────────── */
    {
      ...
      questions: [
        {
          ...
          options: [
            { id: "a", text: "SRAM" },
            { id: "b", text: "DRAM" },
            { id: "c", text: "Flash NAND" },
            { id: "d", text: "Register file" },
          ],
          correctOptionId: "b",
          explanation:
            "DRAM (Dynamic RAM) uses a 1T1C (one transistor, one capacitor) cell. The capacitor leaks charge and must be refreshed approximately every 64 ms. SRAM uses six transistors and does not require refresh.",
        },
        {
          ...
          skillId: "cpu-gpu-memory-design-101-skill-core",
          options: [
            { id: "a", text: "In exactly one specific cache line" },
            { id: "b", text: "In any of the four lines within one specific set" },
            { id: "c", text: "In any cache line in the entire cache" },
            { id: "d", text: "Only in the first line of each set" },
          ],
          correctOptionId: "b",
          explanation:
            "In an N-way set-associative cache, the index bits select a specific set, and the block can be placed in any of the N ways within that set. A 4-way cache allows four possible positions per set.",
        },
        {
          ...
          skillId: "cpu-gpu-memory-design-101-skill-core",
          options: [
            { id: "a", text: "Frequently used CPU instructions" },
            { id: "b", text: "Branch prediction history" },
            { id: "c", text: "Recently used virtual-to-physical page table entries" },
            { id: "d", text: "Dirty cache line addresses pending write-back" },
          ],
          correctOptionId: "c",
          explanation:
            "The TLB is a small, fast cache that stores recent virtual-to-physical address translations from the page table, avoiding costly page table walks for repeated accesses to the same pages.",
        },
        {
          ...
          options: [
            ...
            { id: "b", text: "Write-back" },
            { id: "c", text: "Write-no-allocate" },
            { id: "d", text: "Write-around" },
          ],
          correctOptionId: "b",
          explanation:
            "Write-back marks the cache line as dirty on writes and only sends the data to the next memory level when the line is evicted. This coalesces multiple writes into a single transfer, significantly reducing bandwidth usage.",
        },
      ],
      quizBlueprint: {
        totalQuestions: 4,
    ...
    /* ──────────────────────────────────────────────
       L08  Parallelism and Performance  (interactive)
    ────────────────────────────────────────────── */
    {
      ...
      interactiveActivities: [
        {
          ...
          buckets: ["Instruction-Level (ILP)", "Thread-Level (TLP)", "Data-Level (DLP)"],
          items: [
            { text: "Superscalar CPU issues ADD and MUL in the same cycle from one thread", bucket: "Instruction-Level (ILP)" },
            { text: "Web server handles requests on 16 separate cores simultaneously", bucket: "Thread-Level (TLP)" },
            { text: "AVX-512 multiplies 16 floats in a single instruction", bucket: "Data-Level (DLP)" },
            { text: "Out-of-order execution reorders independent instructions for earlier execution", bucket: "Instruction-Level (ILP)" },
            { text: "GPU warp applies same shader to 32 pixels in parallel", bucket: "Data-Level (DLP)" },
            { text: "OpenMP spawns 8 threads to parallelize a for-loop across CPU cores", bucket: "Thread-Level (TLP)" },
            { text: "Hyper-Threading runs two threads on one core via SMT", bucket: "Thread-Level (TLP)" },
            { text: "SIMD unit processes four 32-bit integers simultaneously", bucket: "Data-Level (DLP)" },
          ],
        },
      ],
    },
    ...
    /* ──────────────────────────────────────────────
       L09  GPU & Parallelism Checkpoint  (quiz)
    ────────────────────────────────────────────── */
    {
      ...
      questions: [
        {
          ...
          correctOptionId: "c",
          explanation:
            "An NVIDIA warp consists of 32 threads that execute the same instruction simultaneously on different data. AMD GPUs use wavefronts of 32 or 64 threads depending on the architecture generation.",
        },
        {
          ...
          correctOptionId: "b",
          explanation:
            "Amdahl's Law: max speedup = 1 / (1 − P) = 1 / (1 − 0.8) = 1 / 0.2 = 5×. The 20% serial portion limits the theoretical maximum to 5× even with infinite processors.",
        },
        {
          ...
          skillId: "cpu-gpu-memory-design-101-skill-core",
          options: [
            { id: "a", text: "The GPU creates a new warp for each branch path" },
            { id: "b", text: "Threads on different paths are serialized — one path executes while others are masked" },
            { id: "c", text: "The entire warp is terminated and restarted" },
            { id: "d", text: "The GPU switches to CPU execution for the divergent section" },
          ],
          correctOptionId: "b",
          explanation:
            "During warp divergence, the GPU serializes execution by masking off threads not on the current path and executing each path sequentially. This reduces effective parallelism until the paths reconverge.",
        },
        {
          ...
          skillId: "cpu-gpu-memory-design-101-skill-core",
          options: [
            { id: "a", text: "Lower cost per gigabyte" },
            { id: "b", text: "Higher bandwidth with lower power consumption per bit" },
            { id: "c", text: "Simpler manufacturing process" },
            { id: "d", text: "Higher clock frequencies" },
          ],
          correctOptionId: "b",
          explanation:
            "HBM uses vertically stacked dies with through-silicon vias and a very wide bus, achieving higher bandwidth at lower power per bit compared to GDDR. GDDR is cheaper per GB but uses more power and narrower buses at higher clocks.",
        },
      ],
      quizBlueprint: {
        totalQuestions: 4,
    ...
    /* ──────────────────────────────────────────────
       L10  Mastery Quiz: Computer Architecture  (quiz)
    ────────────────────────────────────────────── */
    {
      ...
      questions: [
        {
          ...
          skillId: "cpu-gpu-memory-design-101-skill-core",
          options: [
            { id: "a", text: "0 — forwarding eliminates all stalls" },
            { id: "b", text: "1 stall cycle (one bubble inserted)" },
            { id: "c", text: "2 stall cycles" },
            { id: "d", text: "3 stall cycles" },
          ],
          correctOptionId: "b",
          explanation:
            "A load-use hazard requires one stall cycle because the LOAD result is available at the end of the MEM stage, but the dependent instruction needs it at the beginning of EX. Inserting one bubble allows forwarding from MEM/WB to EX, resolving the hazard.",
        },
        {
          ...
          correctOptionId: "b",
          explanation:
            "Number of lines = cache size / line size = 32,768 / 64 = 512 lines. Number of sets = lines / associativity = 512 / 4 = 128 sets. Eight index bits (2⁷ = 128) would be used to select the set.",
        },
        {
          ...
          skillId: "cpu-gpu-memory-design-101-skill-core",
          options: [
            { id: "a", text: "2 TFLOPS (memory-bound)" },
            { id: "b", text: "15 TFLOPS (compute-bound)" },
            { id: "c", text: "30 TFLOPS (compute-bound)" },
            { id: "d", text: "60 TFLOPS (beyond the roofline)" },
          ],
          correctOptionId: "a",
          explanation:
            "The ridge point is peak compute / peak bandwidth = 30 TFLOPS / 1 TB/s = 30 FLOPS/byte. At arithmetic intensity 2 FLOPS/byte, the kernel is memory-bound: attainable performance = bandwidth × intensity = 1 TB/s × 2 FLOPS/byte = 2 TFLOPS.",
        },
        {
          ...
          skillId: "cpu-gpu-memory-design-101-skill-core",
          options: [
            { id: "a", text: "Core B reads from main memory; Core A's line remains Modified" },
            { id: "b", text: "Core A writes the line back to memory and transitions to Shared; Core B receives the data and enters Shared" },
            { id: "c", text: "Core A invalidates its line; Core B gets Exclusive state" },
            { id: "d", text: "Both cores enter Invalid state and must re-fetch from memory" },
          ],
          correctOptionId: "b",
          explanation:
            "Under MESI, when another core requests a read on a Modified line, the owning core writes the dirty data back to memory (or supplies it directly via cache-to-cache transfer) and both cores transition to Shared state, ensuring coherence.",
        },
      ],
      quizBlueprint: {
        totalQuestions: 4,
