...
export const BusinessAnalytics301Module: LearningModule = {
  ...
  description:
    "Harness the power of predictive models and machine learning to anticipate customer behavior, forecast demand, and uncover hidden patterns. Learn classification, clustering, time series, recommendation systems, and text analytics with a business-first approach.",
  subject: "Business",
      ...
      chunks: [
        {
          ...
          content:
            "When the outcome is binary — will a customer churn (yes/no), will a loan default (yes/no) — linear regression fails because it can predict values outside 0 and 1. Logistic regression solves this by using the logistic (sigmoid) function to constrain predictions between 0 and 1, outputting a probability. If the predicted probability exceeds a threshold (typically 0.5), the observation is classified into the positive class."
        },
        {
        ...
        },
        {
          ...
          content:
            "Logistic regression powers many business decisions: credit scoring (will this applicant default?), customer churn prediction (who will cancel?), marketing response modeling (who will respond to this campaign?), and fraud detection (is this transaction fraudulent?). Its transparency — you can see which factors drive the prediction — makes it preferred in regulated industries where model explainability is required."
        }
      ],
      flashcards: [
        { id: "business-analytics-301-l01-f1", front: "Logistic regression", back: "A classification model that uses the sigmoid function to predict the probability of a binary outcome (0 or 1) — outputs values constrained between 0 and 1" },
        { id: "business-analytics-301-l01-f2", front: "Odds ratio in logistic regression", back: "The exponentiated coefficient — an odds ratio of 1.4 means a one-unit increase in the predictor makes the outcome 1.4 times more likely" },
        { id: "business-analytics-301-l01-f3", front: "Why logistic regression is preferred in regulated industries", back: "Its coefficients are interpretable — stakeholders and regulators can see which factors drive predictions, unlike black-box models" }
      ]
    },
    {
      ...
      chunks: [
        ...
        },
        {
          ...
          content:
            "A deep, complex tree can memorize training data perfectly but fail on new data — this is overfitting. The tree captures noise instead of signal. Pruning strategies include setting a maximum depth, requiring a minimum number of observations per leaf, or using cost-complexity pruning that penalizes tree size. The goal is a tree simple enough to generalize but complex enough to capture real patterns."
        },
        {
          id: "business-analytics-301-l02-c3",
          title: "Random Forests",
          content:
            "Random forests build hundreds of decision trees, each trained on a random subset of data and features, then aggregate their predictions through voting (classification) or averaging (regression). This ensemble approach reduces overfitting and improves accuracy. The tradeoff is interpretability — while a single tree is easy to explain, a forest of 500 trees is a black box. Feature importance scores help identify which variables the forest relies on most."
        }
      ],
      flashcards: [
        { id: "business-analytics-301-l02-f1", front: "Gini impurity", back: "A metric measuring how often a randomly chosen element would be misclassified — decision trees select splits that minimize Gini impurity to create purer subgroups" },
        { id: "business-analytics-301-l02-f2", front: "Overfitting in decision trees", back: "When a tree is too deep and memorizes training data noise — it performs well on training data but poorly on unseen data. Prevented by pruning and depth limits." },
        { id: "business-analytics-301-l02-f3", front: "Random forest", back: "An ensemble of hundreds of decision trees trained on random data and feature subsets — aggregates predictions via voting or averaging to reduce overfitting and improve accuracy" }
      ]
    },
    {
      ...
      chunks: [
        ...
        },
        {
          ...
          content:
            "The key question in K-means is how many clusters to use. The elbow method plots the within-cluster sum of squares (WCSS) against K. As K increases, WCSS decreases. The 'elbow' — where adding another cluster yields diminishing improvement — suggests the optimal K. The silhouette score offers another perspective, measuring how similar each point is to its own cluster versus neighboring clusters. Business context should also guide K — three segments may be more actionable than seven."
        },
        {
          id: "business-analytics-301-l03-c3",
          title: "From Clusters to Business Segments",
          content:
            "Raw clusters become valuable when interpreted in business terms. If Cluster 1 has high purchase frequency, high average order value, and long tenure, label them 'Loyal High-Value Customers.' Cluster 2 might be 'Price-Sensitive Bargain Hunters' with low margins but high volume. Each segment then receives a tailored strategy — loyalty rewards for high-value, promotions for price-sensitive, onboarding campaigns for new customers. The analytics only matter if they drive differentiated action."
        }
      ],
      flashcards: [
        { id: "business-analytics-301-l03-f1", front: "K-means clustering", back: "An unsupervised algorithm that partitions data into K groups by iteratively assigning points to the nearest centroid and recalculating centroids until convergence" },
        { id: "business-analytics-301-l03-f2", front: "Elbow method", back: "A technique for choosing the optimal K in K-means — plot WCSS vs. K and select the point where additional clusters yield diminishing improvement (the 'elbow')" },
        { id: "business-analytics-301-l03-f3", front: "Translating clusters to segments", back: "Interpreting cluster characteristics in business terms (e.g., 'High-Value Loyal' or 'Price-Sensitive') and designing differentiated strategies for each group" }
      ]
    },
    {
      ...
      chunks: [
        ...
        },
        {
          ...
          content:
            "Moving averages smooth out noise by averaging the last N observations. A 12-month moving average removes annual seasonality. Exponential smoothing (ETS) improves on this by giving more weight to recent observations — the smoothing parameter α controls how quickly the model adapts. Holt-Winters extends ETS to handle both trend and seasonality simultaneously. These methods are simple, interpretable, and often surprisingly effective for business forecasting."
        },
        {
      ...
      flashcards: [
        { id: "business-analytics-301-l04-f1", front: "Three components of a time series", back: "Trend (long-term direction), seasonality (repeating fixed-interval patterns), and residual (unexplained random variation)" },
        { id: "business-analytics-301-l04-f2", front: "Exponential smoothing", back: "A forecasting method that weights recent observations more heavily — the smoothing parameter α controls adaptation speed. Holt-Winters extends it to handle trend and seasonality." },
        { id: "business-analytics-301-l04-f3", front: "ARIMA(p,d,q)", back: "AutoRegressive Integrated Moving Average — p is the AR order (past values), d is differencing (stationarity), q is MA order (past errors). Captures complex temporal patterns." }
      ]
    },
    {
      ...
      questions: [
        ...
        },
        {
          ...
          options: [
            ...
            { id: "b", text: "Insufficient training data" },
            { id: "c", text: "Overfitting" },
            { id: "d", text: "Using the wrong target variable" }
          ],
          correctOptionId: "c",
        ...
        },
        {
          ...
          options: [
            { id: "a", text: "The best features to include" },
            { id: "b", text: "The optimal number of clusters (K)" },
            { id: "c", text: "Whether the data is normally distributed" },
            { id: "d", text: "The correct distance metric to use" }
          ],
          correctOptionId: "b",
        ...
        },
        {
          ...
          options: [
            { id: "a", text: "Trend" },
            { id: "b", text: "Residual noise" },
            { id: "c", text: "Seasonality" },
            { id: "d", text: "Autocorrelation" }
          ],
          correctOptionId: "c",
    ...
    },
    {
      ...
      chunks: [
        ...
        },
        {
          ...
          content:
            "Accuracy (correct predictions / total) can be misleading with imbalanced data — a model predicting 'no fraud' on 99% non-fraud data achieves 99% accuracy while catching zero fraud. Precision measures what fraction of positive predictions were correct (avoiding false alarms). Recall measures what fraction of actual positives were caught (avoiding misses). The F1 score balances precision and recall. In fraud detection, high recall matters most; in email marketing, high precision avoids annoying customers with irrelevant messages."
        }
      ],
      flashcards: [
        { id: "business-analytics-301-l06-f1", front: "Cold-start problem", back: "The challenge collaborative filtering faces with new users or items — no historical interaction data exists to generate recommendations, requiring content-based or popularity-based fallbacks" },
        { id: "business-analytics-301-l06-f2", front: "Sentiment analysis", back: "An NLP technique that classifies text (reviews, social media, emails) as positive, negative, or neutral — extracts structured insights from unstructured text data" },
        { id: "business-analytics-301-l06-f3", front: "Precision vs. recall", back: "Precision: fraction of positive predictions that were correct (minimizes false alarms). Recall: fraction of actual positives that were caught (minimizes misses). Choose based on business cost of each error type." }
      ]
    },
    {
      ...
      questions: [
        ...
        },
        {
          ...
          options: [
            { id: "a", text: "Using a simpler model that is easier to interpret" },
            { id: "b", text: "Training multiple trees on random subsets and aggregating predictions to reduce overfitting" },
            { id: "c", text: "Eliminating the need for training data" },
            { id: "d", text: "Always selecting the deepest possible tree" }
          ],
          correctOptionId: "b",
        ...
        },
        {
          ...
          options: [
            { id: "a", text: "Select the machine learning algorithm" },
            { id: "b", text: "Collect and clean the data" },
            { id: "c", text: "Define the business question and success criteria" },
            { id: "d", text: "Deploy the model to production" }
          ],
          correctOptionId: "c",
