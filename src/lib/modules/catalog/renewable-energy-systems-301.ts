import type { LearningModule } from "@/lib/modules/types";

export const RenewableEnergySystems301Module: LearningModule = {
  "id": "renewable-energy-systems-301",
  "title": "Renewable Energy Optimization",
  "description": "Advanced optimization of renewable portfolios, forecasting, controls, and reliability planning.",
  "subject": "Renewable Energy",
  "tags": [
    "curriculum",
    "interactive"
  ],
  "minAge": 16,
  "maxAge": 99,
  "version": "1.0.0",
  "difficultyBand": "advanced",
  "localeSupport": [
    "en",
    "es",
    "fr",
    "de",
    "ar",
    "hi",
    "zh",
    "ja",
    "ko",
    "ru"
  ],
  "thumbnail": "/placeholders/lesson-robot.svg",
  "learningObjectives": [
    "Understand core concepts in Renewable Energy",
    "Apply Renewable Energy skills through guided practice",
    "Demonstrate mastery through checkpoint quizzes"
  ],
  "lessons": [
    {
      "id": "renewable-energy-systems-301-l01",
      "title": "Renewable Energy Concepts 1",
      "type": "video",
      "duration": 9,
      "learningAids": [
        {
          "id": "renewable-energy-systems-301-l01-a1",
          "type": "image",
          "title": "Concept Card",
          "content": "Visual summary for Renewable Energy."
        },
        {
          "id": "renewable-energy-systems-301-l01-a2",
          "type": "animation",
          "title": "Animated Example",
          "content": "Step-by-step walkthrough for Renewable Energy."
        }
      ]
    },
    {
      "id": "renewable-energy-systems-301-l02",
      "title": "Renewable Energy Practice 2",
      "type": "interactive",
      "duration": 12,
      "metadata": {
        "prompts": [
          "State the key concept in one sentence.",
          "Apply it to one example.",
          "Explain your improvement step for next time."
        ]
      },
      "learningAids": [
        {
          "id": "renewable-energy-systems-301-l02-a1",
          "type": "practice",
          "title": "Guided Practice",
          "content": "Follow the challenge flow and record your approach."
        }
      ]
    },
    {
      "id": "renewable-energy-systems-301-l03",
      "title": "Checkpoint 1: Renewable Energy",
      "type": "quiz",
      "duration": 10,
      "questions": [
        {
          "id": "renewable-energy-systems-301-l03-q1",
          "text": "Which choice best matches the main idea of Renewable Energy?",
          "skillId": "renewable-energy-systems-301-skill-core",
          "options": [
            {
              "id": "a",
              "text": "A complete, evidence-based explanation"
            },
            {
              "id": "b",
              "text": "An unrelated claim"
            },
            {
              "id": "c",
              "text": "A random guess"
            },
            {
              "id": "d",
              "text": "A conflicting idea"
            }
          ],
          "correctOptionId": "a"
        },
        {
          "id": "renewable-energy-systems-301-l03-q2",
          "text": "What should a learner do after getting an answer wrong?",
          "skillId": "renewable-energy-systems-301-skill-review",
          "options": [
            {
              "id": "a",
              "text": "Review error cause and retry with strategy"
            },
            {
              "id": "b",
              "text": "Skip all corrections"
            },
            {
              "id": "c",
              "text": "Memorize only the answer letter"
            },
            {
              "id": "d",
              "text": "Stop practicing"
            }
          ],
          "correctOptionId": "a"
        },
        {
          "id": "renewable-energy-systems-301-l03-q3",
          "text": "Which strategy most improves long-term retention for Renewable Energy Optimization?",
          "skillId": "renewable-energy-systems-301-skill-core",
          "options": [
            {
              "id": "a",
              "text": "Use spaced retrieval and mixed practice over multiple sessions."
            },
            {
              "id": "b",
              "text": "Cram once and avoid later practice."
            },
            {
              "id": "c",
              "text": "Read notes passively without self-testing."
            },
            {
              "id": "d",
              "text": "Rely on confidence alone instead of evidence."
            }
          ],
          "correctOptionId": "a",
          "explanation": "Spaced retrieval with varied practice strengthens durable memory and flexible application."
        }
      ],
      "learningAids": [
        {
          "id": "renewable-energy-systems-301-l03-a1",
          "type": "mnemonic",
          "title": "Memory Cue",
          "content": "Use Plan, Solve, Explain to structure each response."
        }
      ]
    },
    {
      "id": "renewable-energy-systems-301-l04",
      "title": "Renewable Energy Practice 4",
      "type": "interactive",
      "duration": 12,
      "metadata": {
        "prompts": [
          "State the key concept in one sentence.",
          "Apply it to one example.",
          "Explain your improvement step for next time."
        ]
      },
      "learningAids": [
        {
          "id": "renewable-energy-systems-301-l04-a1",
          "type": "practice",
          "title": "Guided Practice",
          "content": "Follow the challenge flow and record your approach."
        }
      ]
    },
    {
      "id": "renewable-energy-systems-301-l05",
      "title": "Renewable Energy Concepts 5",
      "type": "video",
      "duration": 9,
      "learningAids": [
        {
          "id": "renewable-energy-systems-301-l05-a1",
          "type": "image",
          "title": "Concept Card",
          "content": "Visual summary for Renewable Energy."
        },
        {
          "id": "renewable-energy-systems-301-l05-a2",
          "type": "animation",
          "title": "Animated Example",
          "content": "Step-by-step walkthrough for Renewable Energy."
        }
      ]
    },
    {
      "id": "renewable-energy-systems-301-l06",
      "title": "Checkpoint 2: Renewable Energy",
      "type": "quiz",
      "duration": 10,
      "questions": [
        {
          "id": "renewable-energy-systems-301-l06-q1",
          "text": "Which choice best matches the main idea of Renewable Energy?",
          "skillId": "renewable-energy-systems-301-skill-core",
          "options": [
            {
              "id": "a",
              "text": "A complete, evidence-based explanation"
            },
            {
              "id": "b",
              "text": "An unrelated claim"
            },
            {
              "id": "c",
              "text": "A random guess"
            },
            {
              "id": "d",
              "text": "A conflicting idea"
            }
          ],
          "correctOptionId": "a"
        },
        {
          "id": "renewable-energy-systems-301-l06-q2",
          "text": "What should a learner do after getting an answer wrong?",
          "skillId": "renewable-energy-systems-301-skill-review",
          "options": [
            {
              "id": "a",
              "text": "Review error cause and retry with strategy"
            },
            {
              "id": "b",
              "text": "Skip all corrections"
            },
            {
              "id": "c",
              "text": "Memorize only the answer letter"
            },
            {
              "id": "d",
              "text": "Stop practicing"
            }
          ],
          "correctOptionId": "a"
        },
        {
          "id": "renewable-energy-systems-301-l06-q3",
          "text": "Which strategy most improves long-term retention for Renewable Energy Optimization?",
          "skillId": "renewable-energy-systems-301-skill-core",
          "options": [
            {
              "id": "a",
              "text": "Use spaced retrieval and mixed practice over multiple sessions."
            },
            {
              "id": "b",
              "text": "Cram once and avoid later practice."
            },
            {
              "id": "c",
              "text": "Read notes passively without self-testing."
            },
            {
              "id": "d",
              "text": "Rely on confidence alone instead of evidence."
            }
          ],
          "correctOptionId": "a",
          "explanation": "Spaced retrieval with varied practice strengthens durable memory and flexible application."
        }
      ],
      "learningAids": [
        {
          "id": "renewable-energy-systems-301-l06-a1",
          "type": "mnemonic",
          "title": "Memory Cue",
          "content": "Use Plan, Solve, Explain to structure each response."
        }
      ]
    },
    {
      "id": "renewable-energy-systems-301-l07",
      "title": "Renewable Energy Concepts 7",
      "type": "video",
      "duration": 9,
      "learningAids": [
        {
          "id": "renewable-energy-systems-301-l07-a1",
          "type": "image",
          "title": "Concept Card",
          "content": "Visual summary for Renewable Energy."
        },
        {
          "id": "renewable-energy-systems-301-l07-a2",
          "type": "animation",
          "title": "Animated Example",
          "content": "Step-by-step walkthrough for Renewable Energy."
        }
      ]
    },
    {
      "id": "renewable-energy-systems-301-l08",
      "title": "Renewable Energy Practice 8",
      "type": "interactive",
      "duration": 12,
      "metadata": {
        "prompts": [
          "State the key concept in one sentence.",
          "Apply it to one example.",
          "Explain your improvement step for next time."
        ]
      },
      "learningAids": [
        {
          "id": "renewable-energy-systems-301-l08-a1",
          "type": "practice",
          "title": "Guided Practice",
          "content": "Follow the challenge flow and record your approach."
        }
      ]
    },
    {
      "id": "renewable-energy-systems-301-l09",
      "title": "Checkpoint 3: Renewable Energy",
      "type": "quiz",
      "duration": 10,
      "questions": [
        {
          "id": "renewable-energy-systems-301-l09-q1",
          "text": "Which choice best matches the main idea of Renewable Energy?",
          "skillId": "renewable-energy-systems-301-skill-core",
          "options": [
            {
              "id": "a",
              "text": "A complete, evidence-based explanation"
            },
            {
              "id": "b",
              "text": "An unrelated claim"
            },
            {
              "id": "c",
              "text": "A random guess"
            },
            {
              "id": "d",
              "text": "A conflicting idea"
            }
          ],
          "correctOptionId": "a"
        },
        {
          "id": "renewable-energy-systems-301-l09-q2",
          "text": "What should a learner do after getting an answer wrong?",
          "skillId": "renewable-energy-systems-301-skill-review",
          "options": [
            {
              "id": "a",
              "text": "Review error cause and retry with strategy"
            },
            {
              "id": "b",
              "text": "Skip all corrections"
            },
            {
              "id": "c",
              "text": "Memorize only the answer letter"
            },
            {
              "id": "d",
              "text": "Stop practicing"
            }
          ],
          "correctOptionId": "a"
        },
        {
          "id": "renewable-energy-systems-301-l09-q3",
          "text": "Which strategy most improves long-term retention for Renewable Energy Optimization?",
          "skillId": "renewable-energy-systems-301-skill-core",
          "options": [
            {
              "id": "a",
              "text": "Use spaced retrieval and mixed practice over multiple sessions."
            },
            {
              "id": "b",
              "text": "Cram once and avoid later practice."
            },
            {
              "id": "c",
              "text": "Read notes passively without self-testing."
            },
            {
              "id": "d",
              "text": "Rely on confidence alone instead of evidence."
            }
          ],
          "correctOptionId": "a",
          "explanation": "Spaced retrieval with varied practice strengthens durable memory and flexible application."
        }
      ],
      "learningAids": [
        {
          "id": "renewable-energy-systems-301-l09-a1",
          "type": "mnemonic",
          "title": "Memory Cue",
          "content": "Use Plan, Solve, Explain to structure each response."
        }
      ]
    },
    {
      "id": "renewable-energy-systems-301-l10",
      "title": "Checkpoint 4: Renewable Energy",
      "type": "quiz",
      "duration": 10,
      "questions": [
        {
          "id": "renewable-energy-systems-301-l10-q1",
          "text": "Which choice best matches the main idea of Renewable Energy?",
          "skillId": "renewable-energy-systems-301-skill-core",
          "options": [
            {
              "id": "a",
              "text": "A complete, evidence-based explanation"
            },
            {
              "id": "b",
              "text": "An unrelated claim"
            },
            {
              "id": "c",
              "text": "A random guess"
            },
            {
              "id": "d",
              "text": "A conflicting idea"
            }
          ],
          "correctOptionId": "a"
        },
        {
          "id": "renewable-energy-systems-301-l10-q2",
          "text": "What should a learner do after getting an answer wrong?",
          "skillId": "renewable-energy-systems-301-skill-review",
          "options": [
            {
              "id": "a",
              "text": "Review error cause and retry with strategy"
            },
            {
              "id": "b",
              "text": "Skip all corrections"
            },
            {
              "id": "c",
              "text": "Memorize only the answer letter"
            },
            {
              "id": "d",
              "text": "Stop practicing"
            }
          ],
          "correctOptionId": "a"
        },
        {
          "id": "renewable-energy-systems-301-l10-q3",
          "text": "Which strategy most improves long-term retention for Renewable Energy Optimization?",
          "skillId": "renewable-energy-systems-301-skill-core",
          "options": [
            {
              "id": "a",
              "text": "Use spaced retrieval and mixed practice over multiple sessions."
            },
            {
              "id": "b",
              "text": "Cram once and avoid later practice."
            },
            {
              "id": "c",
              "text": "Read notes passively without self-testing."
            },
            {
              "id": "d",
              "text": "Rely on confidence alone instead of evidence."
            }
          ],
          "correctOptionId": "a",
          "explanation": "Spaced retrieval with varied practice strengthens durable memory and flexible application."
        }
      ],
      "learningAids": [
        {
          "id": "renewable-energy-systems-301-l10-a1",
          "type": "mnemonic",
          "title": "Memory Cue",
          "content": "Use Plan, Solve, Explain to structure each response."
        }
      ]
    }
  ]
};
