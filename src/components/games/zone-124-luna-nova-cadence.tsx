...
const LANE_COUNT = 4;
const START_HEARTS = 7;
const MAX_FOCUS = 6;
const SONG_TICKS = 720;
const HIT_LOW = 76;
const HIT_HIGH = 92;
...
export default function Zone124LunaNovaCadence() {
    ...
    (nextResolution: Resolution, finalScore: number) => {
      ...
      setMessage(
        nextResolution === "encore"
          ? "Luna: nova voyage reached encore finale."
          : "Luna: nova voyage dropped out of tempo.",
      );
  ...
  const start = useCallback(() => {
    ...
    setStartedAt(now);
    setMood("happy");
    setMessage("Luna: keep nova in the hit band until finale.");
    void hapticSelection();
  }, [setMessage, setMood]);
    ...
    <div className="relative w-full overflow-hidden rounded-[2.25rem] border border-fuchsia-300/25 bg-slate-950 text-white">
      ...
      <div className="relative z-10 p-6 md:p-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-fuchsia-200">Zone 124</p>
            <h2 className="text-3xl font-black tracking-tight text-white">Luna Nova Cadence</h2>
          </div>
        ...
        <AnimatePresence mode="wait">
          {phase === "ready" ? (
            <motion.section key="ready" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="rounded-3xl border border-fuchsia-100/30 bg-black/35 p-6 text-center">
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="max-w-2xl text-sm text-fuchsia-100">Hit incoming notes in the nova band and hold streak through the full voyage set.</p>
          ...
          {phase === "paused" ? (
            <motion.section key="paused" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-fuchsia-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">Nova Cadence Paused</h3>
          ...
          {phase === "complete" ? (
            <motion.section key="complete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-fuchsia-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">
                {resolution === "encore" ? "Nova Encore" : "Nova Drop"}
              </h3>
