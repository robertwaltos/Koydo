...
function buildChallenge(roundIndex: number): ReactorChallenge {
  const rand = seeded(`zone-106:${roundIndex + 1}`);
...
export default function Zone106SparkReactorRail() {
    ...
    (nextResolution: Resolution, finalScore: number) => {
      ...
      setMessage(
        nextResolution === "charged"
          ...
          : "Spark: sentinel dynamo fractured.",
      );
  ...
  const advanceRound = useCallback(
    (nextScore: number) => {
      ...
      setSelectedRail(nextChallenge.requiredRail);
    },
    [finalize, roundIndex],
  );
  ...
  const start = useCallback(() => {
    ...
    setSelectedRail(firstChallenge.requiredRail);
    ...
    <div className="relative w-full overflow-hidden rounded-[2.25rem] border border-amber-300/25 bg-slate-950 text-white">
      ...
      <div className="relative z-10 p-6 md:p-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-amber-200">Zone 106</p>
            <h2 className="text-3xl font-black tracking-tight text-white">Spark Reactor Rail</h2>
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === "paused" ? (
            <motion.section key="paused" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-amber-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">Reactor Rail Paused</h3>
