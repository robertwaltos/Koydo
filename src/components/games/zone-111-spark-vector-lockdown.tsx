...
function buildChallenge(roundIndex: number): ReactorChallenge {
  const rand = seeded(`zone-111:${roundIndex + 1}`);
...
export default function Zone111SparkVectorLockdown() {
  ...
  const finalize = useCallback(
    (nextResolution: Resolution, finalScore: number) => {
      ...
      setMessage(
        ...
        ? "Spark: vector lockdown fully charged."
        : "Spark: vector lockdown fractured.",
      );
    ...
    <div className="relative w-full overflow-hidden rounded-[2.25rem] border border-amber-300/25 bg-slate-950 text-white">
      ...
      <div className="relative z-10 p-6 md:p-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-amber-200">Zone 111</p>
            <h2 className="text-3xl font-black tracking-tight text-white">Spark Vector Lockdown</h2>
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === "paused" ? (
            <motion.section key="paused" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-amber-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">Vector Lockdown Paused</h3>
          ...
          {phase === "complete" ? (
            <motion.section key="complete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-amber-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">
                {resolution === "charged" ? "Vector Lockdown Charged" : "Vector Lockdown Fracture"}
              </h3>
