...
export default function Zone114LunaHarmonicHyperlane() {
    ...
    (nextResolution: Resolution, finalScore: number) => {
      ...
      setMessage(
        nextResolution === "encore"
          ? "Luna: harmonic voyage reached encore finale."
          : "Luna: harmonic voyage dropped out of tempo.",
      );
  ...
  const start = useCallback(() => {
    ...
    setStartedAt(now);
    setMood("happy");
    setMessage("Luna: keep harmonic in the hit band until finale.");
    void hapticSelection();
  }, [setMessage, setMood]);
    ...
    <div className="relative w-full overflow-hidden rounded-[2.25rem] border border-fuchsia-300/25 bg-slate-950 text-white">
      ...
      <div className="relative z-10 p-6 md:p-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-fuchsia-200">Zone 114</p>
            <h2 className="text-3xl font-black tracking-tight text-white">Luna Harmonic Hyperlane</h2>
          </div>
        ...
        <AnimatePresence mode="wait">
          {phase === "ready" ? (
            <motion.section key="ready" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="rounded-3xl border border-fuchsia-100/30 bg-black/35 p-6 text-center">
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="max-w-2xl text-sm text-fuchsia-100">Hit incoming notes in the harmonic band and hold streak through the full voyage set.</p>
                <PhysicalButton onClick={start} className="h-12 bg-fuchsia-400 px-8 text-sm font-black text-slate-950">
                  Start Harmonic Hyperlane
                </PhysicalButton>
              </div>
            </motion.section>
          ) : null}
          ...
          {phase === "paused" ? (
            <motion.section key="paused" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-fuchsia-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">Harmonic Hyperlane Paused</h3>
          ...
          {phase === "complete" ? (
            <motion.section key="complete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-fuchsia-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">
                {resolution === "encore" ? "Hyperlane Encore" : "Hyperlane Drop"}
              </h3>
