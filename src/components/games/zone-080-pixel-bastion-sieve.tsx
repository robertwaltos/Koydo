...
function buildRound(roundIndex: number): SieveRound {
  const rand = seeded(`zone-080:${roundIndex + 1}`);
  ...
  const prompts = [
    "Bastion sieve flagged synchronized spoof mesh.",
    "Gate cluster reports conflicting route entropy.",
    "Sentinel index sees mirrored surge profile.",
    "Core bastion detected delayed hash replay.",
    "Zero-trust corridor is under layered probe storm.",
  ];

  return {
    id: `z080-r${roundIndex + 1}`,
...
export default function Zone080PixelBastionSieve() {
  ...
  const finalize = (nextResolution: Resolution, finalScore: number) => {
    ...
    setMessage(
      nextResolution === "sealed"
        ? "Pixel: bastion sieve sealed all breach lanes."
        : "Pixel: breach escaped the sieve.",
    );
  ...
  const start = () => {
    ...
    setStartedAt(now);
    setMood("happy");
    setMessage("Pixel: identify the true breach lane, not the decoy.");
  ...
  const patchFirewall = () => {
    ...
    setMood("happy");
    setMessage("Patch routine restored one firewall segment.");
    void hapticSelection();
  };
  ...
  const lockLane = (lane: number) => {
    ...
    if (exact) {
      ...
      if (nextCombo > 0 && nextCombo % 4 === 0) {
        ...
        setMessage("Sieve streak: +1 patch.");
      } else {
      ...
      setMood("sad");
      setMessage(`Bad lock. Breach lane was ${round.breachLane + 1}.`);
      void hapticError();
    }
    ...
    <div className="relative w-full overflow-hidden rounded-[2.25rem] border border-sky-300/25 bg-slate-950 text-white">
      ...
      <div className="relative z-10 p-6 md:p-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-sky-200">Zone 080</p>
            <h2 className="text-3xl font-black tracking-tight text-white">Pixel Bastion Sieve</h2>
          </div>
        ...
        <AnimatePresence mode="wait">
          {phase === "ready" ? (
            <motion.section key="ready" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="rounded-3xl border border-sky-200/25 bg-black/35 p-6 text-center">
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="max-w-2xl text-sm text-sky-100">Complete 16 sieve sweeps and lock the real breach lane under heavy spoof pressure.</p>
          ...
          {phase === "playing" && round ? (
            <motion.section key={round.id} initial={{ opacity: 0.2, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="rounded-3xl border border-sky-100/30 bg-black/35 p-6">
              ...
              <p className="mt-2 text-xs text-sky-200">Channel signature spread:</p>
          ...
          {phase === "paused" ? (
            <motion.section key="paused" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-sky-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">Sieve Paused</h3>
          ...
          {phase === "complete" ? (
            <motion.section key="complete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-sky-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">
                {resolution === "sealed" ? "Bastion Sealed" : "Bastion Breached"}
              </h3>
