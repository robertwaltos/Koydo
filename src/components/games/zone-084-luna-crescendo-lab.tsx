...
function buildWave(waveIndex: number): LabWave {
  const rand = seeded(`zone-084:${waveIndex + 1}`);
  ...
  const prompts = [
    "Crescendo line requires exact replay.",
    "Harmony bridge entered accelerated mode.",
    "Crowd-response loop shifted one beat ahead.",
    "Finale stack demands full focus chain.",
  ];
  return {
    id: `z084-w${waveIndex + 1}`,
    sequence,
...
export default function Zone084LunaCrescendoLab() {
  ...
  const finalize = (nextResolution: Resolution, finalScore: number) => {
    ...
    setMessage(
      nextResolution === "encore"
        ? "Luna: crescendo lab cleared."
        : "Luna: rhythm chain broke before finale.",
    );
  ...
  const start = () => {
    ...
    setStartedAt(now);
    setMood("happy");
    setMessage("Luna: mirror each crescendo sequence exactly.");
    ...
    if (padIndex === expected) {
      ...
      if (nextInputIndex >= wave.sequence.length) {
        ...
        if (nextCombo > 0 && nextCombo % 3 === 0) {
          ...
          setMessage("Crescendo chain perfect. +1 focus.");
        } else {
    ...
    <div className="relative w-full overflow-hidden rounded-[2.25rem] border border-fuchsia-300/25 bg-slate-950 text-white">
      ...
      <div className="relative z-10 p-6 md:p-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-fuchsia-200">Zone 084</p>
            <h2 className="text-3xl font-black tracking-tight text-white">Luna Crescendo Lab</h2>
          </div>
        ...
        <AnimatePresence mode="wait">
          {phase === "ready" ? (
            <motion.section key="ready" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="rounded-3xl border border-fuchsia-200/25 bg-black/35 p-6 text-center">
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="max-w-2xl text-sm text-fuchsia-100">Memorize and replay 13 escalating crescendo waves.</p>
          ...
          {phase === "paused" ? (
            <motion.section key="paused" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-fuchsia-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">Crescendo Paused</h3>
          ...
          {phase === "complete" ? (
            <motion.section key="complete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-fuchsia-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">
                {resolution === "encore" ? "Crescendo Encore" : "Crescendo Break"}
              </h3>
