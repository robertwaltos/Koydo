...
function buildInfection(wave: number): string[] {
  const rand = seeded(`zone-120:wave:${wave}`);
...
function spreadInfection(infectedKeys: string[], shields: Record<string, number>, wave: number, tick: number) {
  const rand = seeded(`zone-120:spread:${wave}:${tick}:${infectedKeys.join("|")}`);
...
export default function Zone120PixelQuarantineGrid() {
    ...
    (nextResolution: Resolution, finalScore: number) => {
      ...
      setMessage(
        nextResolution === "sealed"
          ? "Pixel: quarantine grid operations sealed all breach waves."
          : "Pixel: quarantine grid operations breached at core pressure.",
      );
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (spread.length >= 16 || nextPressure >= 3) {
        ...
        setScore(nextScore);
        setMessage("Containment overload. Quarantine segment lost.");
        setMood("sad");
        void hapticError();
    ...
    <div className="relative w-full overflow-hidden rounded-[2.25rem] border border-sky-300/25 bg-slate-950 text-white">
      ...
      <div className="relative z-10 p-6 md:p-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-sky-200">Zone 120</p>
            <h2 className="text-3xl font-black tracking-tight text-white">Pixel Quarantine Grid</h2>
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === "paused" ? (
            <motion.section key="paused" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-sky-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">Quarantine Grid Paused</h3>
          ...
          {phase === "complete" ? (
            <motion.section key="complete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-sky-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">
                {resolution === "sealed" ? "Quarantine Grid Sealed" : "Quarantine Grid Breached"}
              </h3>
