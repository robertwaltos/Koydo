...
function buildWave(waveIndex: number): ChorusWave {
  const rand = seeded(`zone-079:${waveIndex + 1}`);
  ...
  const prompts = [
    "Prism chorus needs exact mirror timing.",
    "Bridge lights shifted into sync challenge.",
    "Crowd pulse demands perfect recall.",
    "Final stage line has reduced margin.",
  ];
  return {
    id: `z079-w${waveIndex + 1}`,
    sequence,
...
export default function Zone079LunaPrismChorus() {
  ...
  const finalize = (nextResolution: Resolution, finalScore: number) => {
    ...
    setMessage(
      nextResolution === "encore"
        ? "Luna: prism chorus completed."
        : "Luna: prism chorus dropped before finale.",
    );
  ...
  const start = () => {
    ...
    setStartedAt(now);
    setMood("happy");
    setMessage("Luna: watch the chorus pattern and mirror it perfectly.");
  ...
  const focusBreath = () => {
    ...
    setMood("happy");
    setMessage("Focus breath restored one heart.");
    void hapticSelection();
  };
  ...
  const pressPad = (padIndex: number) => {
    ...
    if (padIndex === expected) {
      ...
      if (nextInputIndex >= wave.sequence.length) {
        ...
        if (nextCombo > 0 && nextCombo % 3 === 0) {
          ...
          setMessage("Perfect chain. +1 focus.");
        } else {
          setMessage("Chorus segment clean.");
        }
    ...
    if (nextFocus > 0) {
      ...
      setMood("thinking");
      setMessage("Focus absorbed the miss. Replay sequence.");
      void hapticSelection();
    } else {
  ...
  useEffect(() => {
    ...
    setMessage("Watch the prism sequence.");
  ...
  }, [phase, pressPad, focusBreath]);

  return (
    <div className="relative w-full overflow-hidden rounded-[2.25rem] border border-fuchsia-300/25 bg-slate-950 text-white">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_16%_18%,rgba(232,121,249,0.26),transparent_44%),radial-gradient(circle_at_84%_82%,rgba(217,70,239,0.2),transparent_52%)]" />
      <div className="relative z-10 p-6 md:p-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-fuchsia-200">Zone 079</p>
            <h2 className="text-3xl font-black tracking-tight text-white">Luna Prism Chorus</h2>
          </div>
          ...
          {phase === "ready" ? (
            <motion.section key="ready" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="rounded-3xl border border-fuchsia-200/25 bg-black/35 p-6 text-center">
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="max-w-2xl text-sm text-fuchsia-100">Memorize and replay 13 chorus waves with accelerating tempo.</p>
          ...
          {(phase === "preview" || phase === "input") && wave ? (
            <motion.section key={wave.id} initial={{ opacity: 0.2, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="rounded-3xl border border-fuchsia-100/30 bg-black/35 p-6">
              ...
              <p className="mt-2 text-xs text-fuchsia-200">
                {phase === "preview" ? "Preview: watch and memorize." : `Input: ${inputIndex}/${wave.sequence.length} notes entered.`}
              </p>
          ...
          {phase === "complete" ? (
            <motion.section key="complete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-fuchsia-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">
                {resolution === "encore" ? "Prism Encore" : "Chorus Drop"}
              </h3>
