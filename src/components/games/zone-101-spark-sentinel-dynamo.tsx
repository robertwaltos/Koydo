...
function buildRound(roundIndex: number): ReactorRound {
  const rand = seeded(`zone-101:${roundIndex + 1}`);
  const tier = 1 + Math.floor(roundIndex / 4);
  const targetPulse = 43 + Math.floor(rand() * 20) + tier;
  const targetCoolant = 42 + Math.floor(rand() * 20) + Math.floor(tier / 2);
  const swing = 5 + tier * 2;
  const driftPulse = Math.floor(rand() * (swing * 2 + 1)) - swing;
  const driftCoolant = Math.floor(rand() * (swing * 2 + 1)) - swing;
  const base = 4 + tier;
  const options: PulseOption[] = [
    {
      label: "Pulse overclock",
      pulseAdjust: base + 2,
  ...
  const prompts = [
    "sentinel dynamo rails drifted from escort lock.",
    "Coolant cycle lag detected in core channel.",
    "Pulse manifold and thermal sleeve diverged.",
    "Containment shell saw uneven surge pressure.",
    "Reactor output pattern shifted mid-load.",
  ];
  return {
    id: `z101-r${roundIndex + 1}`,
    prompt: `Pulse cycle ${roundIndex + 1}: ${prompts[Math.floor(rand() * prompts.length)] ?? prompts[0]}`,
    targetPulse,
    targetCoolant,
    driftPulse,
    driftCoolant,
...
export default function Zone101SparkSentinelDynamo() {
  const { setMessage, setMood } = useMascot();
  const [phase, setPhase] = useState<Phase>("ready");
  const [roundIndex, setRoundIndex] = useState(0);
  const [pulse, setPulse] = useState(54);
  ...
  const finalize = (nextResolution: Resolution, finalScore: number) => {
    ...
    setMessage(
      nextResolution === "charged"
        ? "Spark: sentinel dynamo fully charged."
        : "Spark: sentinel dynamo fractured containment.",
    );
  ...
  const start = () => {
    ...
    setStartedAt(now);
    setMood("happy");
    setMessage("Spark: keep pulse and coolant aligned through all cycles.");
  ...
  const stabilize = () => {
    ...
    setPulse((value) => clamp(value + Math.round((52 - value) * 0.3), 0, 100));
  ...
  const choose = (optionIndex: number) => {
    ...
    const nextPulse = clamp(pulse + round.driftPulse + option.pulseAdjust, 0, 100);
    const nextCoolant = clamp(coolant + round.driftCoolant + option.coolantAdjust, 0, 100);
    const pulseError = Math.abs(nextPulse - round.targetPulse);
    const coolantError = Math.abs(nextCoolant - round.targetCoolant);
    const success = pulseError <= tolerance && coolantError <= tolerance;
    const perfect = pulseError <= Math.max(3, Math.floor(tolerance / 2)) && coolantError <= Math.max(3, Math.floor(tolerance / 2));
    const severe = pulseError >= tolerance + 8 || coolantError >= tolerance + 8;
    ...
    if (success) {
      ...
      if (perfect) {
        ...
        setMessage("Perfect pulse lock. +1 reserve.");
      } else {
        setMessage("Citadel cycle stabilized.");
      }
      setMood("happy");
      void hapticSuccess();
      ...
      setMood("sad");
      setMessage("Core strain exceeded safe limits.");
      void hapticError();
    }
    ...
    <div className="relative w-full overflow-hidden rounded-[2.25rem] border border-amber-300/25 bg-slate-950 text-white">
      ...
      <div className="relative z-10 p-6 md:p-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-amber-200">Zone 101</p>
            <h2 className="text-3xl font-black tracking-tight text-white">Spark Sentinel Dynamo</h2>
          </div>
        ...
        <AnimatePresence mode="wait">
          {phase === "ready" ? (
            <motion.section key="ready" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="rounded-3xl border border-amber-200/25 bg-black/35 p-6 text-center">
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="max-w-2xl text-sm text-amber-100">Run 16 sentinel dynamo cycles and hold containment through final charge.</p>
            ...
            <motion.section key={round.id} initial={{ opacity: 0.2, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="rounded-3xl border border-amber-100/30 bg-black/35 p-6">
              ...
              <div className="mt-3 grid gap-2 text-xs text-amber-100 sm:grid-cols-2">
                <p className="rounded-xl border border-amber-100/25 bg-amber-100/10 px-3 py-2">
                  Pulse {pulse} | Target {round.targetPulse} | Drift {round.driftPulse >= 0 ? "+" : ""}{round.driftPulse}
                </p>
                <p className="rounded-xl border border-amber-100/25 bg-amber-100/10 px-3 py-2">
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="mt-1 text-xs text-amber-100">
                        P {option.pulseAdjust >= 0 ? "+" : ""}{option.pulseAdjust} | C {option.coolantAdjust >= 0 ? "+" : ""}{option.coolantAdjust}
                      </p>
                    </button>
                  );
                })}
          ...
          {phase === "paused" ? (
            <motion.section key="paused" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-amber-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">Sentinel Run Paused</h3>
          ...
          {phase === "complete" ? (
            <motion.section key="complete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-amber-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">
                {resolution === "charged" ? "Sentinel Dynamo Charged" : "Sentinel Dynamo Fracture"}
              </h3>
