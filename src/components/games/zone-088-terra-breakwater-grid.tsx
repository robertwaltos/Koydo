...
export default function Zone088TerraBreakwaterGrid() {
  ...
  const finalize = (nextResolution: Resolution, finalScore: number) => {
    ...
    setMessage(
      nextResolution === "secured"
        ? "Terra: breakwater grid secured."
        : "Terra: breakwater grid breached.",
    );
  ...
  useEffect(() => {
    ...
    const loop = window.setInterval(() => {
      ...
      if (breached) {
        setMood("sad");
        setMessage("Breakwater grid breached a protection gate.");
      }
    ...
    <div className="relative w-full overflow-hidden rounded-[2.25rem] border border-emerald-300/25 bg-slate-950 text-white">
      ...
      <div className="relative z-10 p-6 md:p-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-emerald-200">Zone 088</p>
            <h2 className="text-3xl font-black tracking-tight text-white">Terra Breakwater Grid</h2>
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === "paused" ? (
            <motion.section key="paused" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-emerald-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">Breakwater Paused</h3>
