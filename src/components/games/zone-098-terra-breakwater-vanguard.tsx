...
type SurgeKind = "tide" | "debris" | "storm" | "supply";
...
export default function Zone098TerraBreakwaterVanguard() {
  ...
  const finalize = (nextResolution: Resolution, finalScore: number) => {
    ...
    setMessage(
      nextResolution === "secured"
        ? "Terra: breakwater vanguard secured."
        : "Terra: breakwater vanguard breached.",
    );
  ...
  const start = () => {
    ...
    setStartedAt(now);
    setMood("happy");
    setMessage("Terra: intercept incoming surges and capture supply pods.");
  ...
  useEffect(() => {
    ...
    const loop = window.setInterval(() => {
      ...
      if (Math.random() < spawnChance) {
        ...
        const kind: SurgeKind =
          Math.random() < hostileChance
            ? (["tide", "debris", "storm"][Math.floor(Math.random() * 3)] as SurgeKind)
            : "supply";
      ...
      if (breached) {
        setMood("sad");
        setMessage("breakwater vanguard breached a protection gate.");
      }
    ...
    <div className="relative w-full overflow-hidden rounded-[2.25rem] border border-emerald-300/25 bg-slate-950 text-white">
      ...
      <div className="relative z-10 p-6 md:p-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-emerald-200">Zone 098</p>
            <h2 className="text-3xl font-black tracking-tight text-white">Terra Breakwater Vanguard</h2>
          </div>
        ...
        <AnimatePresence mode="wait">
          {phase === "ready" ? (
            <motion.section key="ready" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="rounded-3xl border border-emerald-200/25 bg-black/35 p-6 text-center">
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="max-w-2xl text-sm text-emerald-100">Defend four breakwater lanes and keep barriers intact through full storm window.</p>
                <PhysicalButton onClick={start} className="h-12 bg-emerald-400 px-8 text-sm font-black text-slate-950">
                  Start Breakwater Vanguard
                </PhysicalButton>
              </div>
            </motion.section>
          ) : null}
          ...
          {phase === "paused" ? (
            <motion.section key="paused" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-emerald-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">Breakwater Vanguard Paused</h3>
          ...
          {phase === "complete" ? (
            <motion.section key="complete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-emerald-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">
                {resolution === "secured" ? "Breakwater Secured" : "Breakwater Breach"}
              </h3>
