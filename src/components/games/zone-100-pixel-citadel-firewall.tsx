...
function buildRound(roundIndex: number): WeaveRound {
  const rand = seeded(`zone-100:${roundIndex + 1}`);
  ...
  const prompts = [
    "Aegis weave caught synchronized spoof strands.",
    "Edge lattice sees mirrored signal pressure.",
    "Gateway fabric flagged staggered replay bursts.",
    "Core weave reports inconsistent route entropy.",
    "Sentinel curtain identified layered decoy traffic.",
  ];

  return {
    id: `z100-r${roundIndex + 1}`,
...
export default function Zone100PixelCitadelFirewall() {
  ...
  const finalize = (nextResolution: Resolution, finalScore: number) => {
    ...
    setMessage(
      nextResolution === "sealed"
        ? "Pixel: citadel firewall sealed the network."
        : "Pixel: spoof breach pierced the citadel firewall.",
    );
  ...
  const start = () => {
    ...
    setStartedAt(now);
    setMood("happy");
    setMessage("Pixel: lock true breach lanes and ignore decoy signatures.");
  ...
  const lockLane = (lane: number) => {
    ...
    if (exact) {
      ...
      if (nextCombo > 0 && nextCombo % 4 === 0) {
        ...
        setMessage("Firewall chain secured. +1 patch.");
      } else {
    ...
    <div className="relative w-full overflow-hidden rounded-[2.25rem] border border-sky-300/25 bg-slate-950 text-white">
      ...
      <div className="relative z-10 p-6 md:p-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-sky-200">Zone 100</p>
            <h2 className="text-3xl font-black tracking-tight text-white">Pixel Citadel Firewall</h2>
          </div>
        ...
        <AnimatePresence mode="wait">
          {phase === "ready" ? (
            <motion.section key="ready" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="rounded-3xl border border-sky-200/25 bg-black/35 p-6 text-center">
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="max-w-2xl text-sm text-sky-100">Run 16 citadel firewall sweeps and stop every spoof breach before core lanes.</p>
                <PhysicalButton onClick={start} className="h-12 bg-sky-400 px-8 text-sm font-black text-slate-950">
                  Start Firewall Sweep
                </PhysicalButton>
              </div>
            </motion.section>
          ) : null}
          ...
          {phase === "playing" && round ? (
            <motion.section key={round.id} initial={{ opacity: 0.2, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="rounded-3xl border border-sky-100/30 bg-black/35 p-6">
              ...
              <p className="mt-2 text-xs text-sky-200">Guardian channel pressure:</p>
          ...
          {phase === "paused" ? (
            <motion.section key="paused" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-sky-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">Firewall Sweep Paused</h3>
          ...
          {phase === "complete" ? (
            <motion.section key="complete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-sky-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">
                {resolution === "sealed" ? "citadel firewall Sealed" : "citadel firewall Breached"}
              </h3>
