...
export default function Zone113TerraOutpostRecovery() {
    ...
    (nextResolution: Resolution, finalScore: number) => {
      ...
      setMessage(
        nextResolution === "rescued"
          ? "Terra: all outpost pods rescued."
          : "Terra: outpost grid collapsed.",
      );
  ...
  const spawnPod = useCallback(
    (nextHazards: Coord[], rescueCount: number) => {
      ...
      return randomCell(exclusion, `zone-113:pod:${rescueCount}:${tickRef.current}`);
    },
    [],
  );
  ...
  const start = useCallback(() => {
    tickRef.current = 0;
    const firstHazards = spawnHazards(3, new Set([keyOf(BASE)]), "zone-113:start");
  ...
  const step = useCallback(
    (dx: number, dy: number) => {
      ...
      const movedHazards = moveHazards(hazards, `zone-113:move:${tickRef.current}`);
      ...
      if (nextCarrying && sameCoord(nextRover, BASE)) {
        ...
        if (nextCombo % 3 === 0 && nextHull < START_HULL) {
          ...
          setMessage("Pod delivered to outpost base.");
        }
        setMood("happy");
        void hapticSuccess();
    ...
    <div className="relative w-full overflow-hidden rounded-[2.25rem] border border-emerald-300/25 bg-slate-950 text-white">
      ...
      <div className="relative z-10 p-6 md:p-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-emerald-200">Zone 113</p>
            <h2 className="text-3xl font-black tracking-tight text-white">Terra Outpost Recovery</h2>
          </div>
        ...
        <AnimatePresence mode="wait">
          {phase === "ready" ? (
            <motion.section key="ready" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="rounded-3xl border border-emerald-100/30 bg-black/35 p-6 text-center">
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="max-w-2xl text-sm text-emerald-100">Navigate storm hazards, pick up outpost pods, and return each to base.</p>
                <PhysicalButton onClick={start} className="h-12 bg-emerald-400 px-8 text-sm font-black text-slate-950">
                  Start Outpost Recovery
                </PhysicalButton>
              </div>
            </motion.section>
          ) : null}
          ...
          {phase === "paused" ? (
            <motion.section key="paused" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-emerald-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">Outpost Recovery Paused</h3>
          ...
          {phase === "complete" ? (
            <motion.section key="complete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-emerald-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">
                {resolution === "rescued" ? "Outpost Pods Recovered" : "Outpost Collapse"}
              </h3>
