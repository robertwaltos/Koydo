...
type Tribunal = "accept" | "challenge" | "defer" | "escalate";
...
type CircuitRound = {
  ...
  urgency: number;
  options: [Tribunal, Tribunal, Tribunal, Tribunal];
  expected: Tribunal;
};
...
const VERDICT_LABELS: Record<Tribunal, string> = {
...
function rotateTribunals(offset: number): [Tribunal, Tribunal, Tribunal, Tribunal] {
  const base: Tribunal[] = ["accept", "challenge", "defer", "escalate"];
...
function expectedTribunal(clarity: number, contradiction: number, urgency: number): Tribunal {
...
function buildRound(roundIndex: number): CircuitRound {
  const rand = seeded(`zone-102:${roundIndex + 1}`);
  ...
  const options = rotateTribunals(roundIndex + Math.floor(rand() * 4));
  const expected = expectedTribunal(clarity, contradiction, urgency);
  const prompts = [
    "evidence lattice receives disputed witness chain.",
...
export default function Zone102EchoEvidenceLattice() {
  ...
  const finalize = (nextResolution: Resolution, finalScore: number) => {
    ...
    setMessage(
      nextResolution === "certified"
        ? "Echo: evidence lattice certified."
        : "Echo: evidence lattice compromised.",
    );
    ...
    <div className="relative w-full overflow-hidden rounded-[2.25rem] border border-cyan-300/25 bg-slate-950 text-white">
      ...
      <div className="relative z-10 p-6 md:p-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-cyan-200">Zone 102</p>
            <h2 className="text-3xl font-black tracking-tight text-white">Echo Evidence Lattice</h2>
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === "paused" ? (
            <motion.section key="paused" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-cyan-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">Evidence Lattice Paused</h3>
          ...
          {phase === "complete" ? (
            <motion.section key="complete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border border-cyan-100/30 bg-black/45 p-6 text-center">
              ...
              <h3 className="mt-3 text-2xl font-black text-white">
                {resolution === "certified" ? "Evidence Lattice Certified" : "Evidence Lattice Compromised"}
              </h3>
